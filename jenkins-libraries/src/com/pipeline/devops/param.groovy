...
def param(config) {
  if ( config.info.image == 'true' ) {
    
    properties([
      parameters([
        gitParameter(name: 'BRANCH', branchFilter: 'origin/(.*)', defaultValue: config.info.branch, selectedValue: 'DEFAULT', listSize: '10', useRepository: config.info.repository_name, type: 'PT_BRANCH', description: '分支/标签' ),
    ...
    properties([
      parameters([
        ...
        text(name: 'CMD', defaultValue: config.info.build_cmd, description: '编译命令'),
        text(name: 'CleanJar', defaultValue: '', description: '清理Maven缓存中的jar <br />一行一条<br />MavenDefault:.m2/repository/<br />只需要填写该目录后续的路径'),
        string(name: 'DebugPort', defaultValue:'' , description: '远程调试端口(注意不要端口冲突)', trim: true)
        ])
    ])
  } else {
    properties([
